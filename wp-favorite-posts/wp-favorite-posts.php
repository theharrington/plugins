...
define('WPFP_COOKIE_KEY_SFL', "wp-saved-posts");
...
function wpfp_list_favorite_posts( $args = array() ) {
    ...
    if ( !empty($user) ) {
        ...
        $saved_post_ids = wpfp_get_users_saved();
    }
...
function wpfp_init() {
    ...
    $wpfp_options['cookie_warning'] = "Your favourite apartments are saved to your browsers cookies. Please note that if you clear your cookies, your favourites will be deleted.";
...
function wpfp_clear_list_link() {
    if (wpfp_is_user_can_edit()) {
        ...
        echo "<a class='wpfp-link' href='?wpfpaction=clear' rel='nofollow'><i class='fa fa-trash'></i>&nbsp;&nbsp;". wpfp_get_option('clear') . "</a>";
    }
}
...
function wpfp_update_user_meta_sfl($arr) {
    return update_user_meta_sfl(wpfp_get_user_id(),WPFP_META_KEY_SFL,$arr);
}

function wpfp_update_post_meta_sfl($post_id, $val) {
	$oldval = wpfp_get_post_meta_sfl($post_id);
	if ($val == -1 && $oldval == 0) {
    	$val = 0;
	} else {
		$val = $oldval + $val;
	}
    return add_post_meta_sfl($post_id, WPFP_META_KEY_SFL, $val, true) or update_post_meta($post_id, WPFP_META_KEY_SFL, $val);
}

function wpfp_delete_post_meta_sfl($post_id) {
    return delete_post_meta($post_id, WPFP_META_KEY);
}

function wpfp_get_user_meta_sfl($user = "") {
    if (!empty($user)):
        $userdata = get_user_by( 'login', $user );
        $user_id = $userdata->ID;
        return get_user_meta($user_id, WPFP_META_KEY_SFL, true);
    else:
        return get_user_meta(wpfp_get_user_id(), WPFP_META_KEY_SFL, true);
    endif;
}

function wpfp_get_users_saved($user = "") {
    $saved_post_ids = array();

    if (!empty($user)):
        return wpfp_get_user_meta_sfl($user);
    endif;

    # collect favorites from cookie and if user is logged in from database.
    if (is_user_logged_in()):
        $saved_post_ids = wpfp_get_user_meta_sfl();
	else:
	    if (wpfp_get_cookie_sfl()):
	        foreach (wpfp_get_cookie_sfl() as $post_id => $post_title) {
	            array_push($saved_post_ids, $post_id);
	        }
	    endif;
	endif;
...
function wpfp_set_cookie_sfl($post_id, $str) {
    $expire = time()+60*60*24*30;
    return setcookie("wp-saved-posts[$post_id]", $str, $expire, "/");
}

function wpfp_get_cookie_sfl() {
    if (!isset($_COOKIE[WPFP_COOKIE_KEY_SFL])) return;
    return $_COOKIE[WPFP_COOKIE_KEY_SFL];
}

function wpfp_save_favorite($post_id = "") {
    if (empty($post_id)) $post_id = $_REQUEST['postid'];
	if(wpfp_do_save_favorite($post_id)){
	}else{return false;}
}

function wpfp_do_save_favorite($post_id) {
    $a = true;
    if (is_user_logged_in()) {
        $user_favorites = wpfp_get_user_meta();
        $user_favorites = array_diff($user_favorites, array($post_id));
    ...
    if ($a) $a = wpfp_set_cookie($_REQUEST['postid'], "");
    if ($a) $a = wpfp_set_cookie_sfl($_REQUEST['postid'], "added");
    return $a;
}

function wpfp_trash_saved($post_id = "") {
    if (empty($post_id)) $post_id = $_REQUEST['postid'];
	if(wpfp_do_trash_saved($post_id)){
	}else{return false;}
}

function wpfp_do_trash_saved($post_id) {
    $a = true;
    if ($a) $a = wpfp_set_cookie_sfl($_REQUEST['postid'], "");
    return $a;
}

function wpfp_restore_favorite_link($post_id) {
    if (wpfp_is_user_can_edit()) {
        $wpfp_options = wpfp_get_options();
        $class = 'wpfp-link remove-parent';
        $link = "<a id='rem_$post_id' class='$class fav-remove' href='?wpfpaction=restore&amp;page=1&amp;postid=". $post_id ."' rel='nofollow'>restore in list</a>";
        // $link = apply_filters( 'wpfp_add_favorite', $link );
        echo $link;
    }
}

function wpfp_trash_favorite_link($post_id) {
    if (wpfp_is_user_can_edit()) {
        $wpfp_options = wpfp_get_options();
        $class = 'wpfp-link remove-parent';
        $link = "<a id='rem_$post_id' class='$class fav-remove' href='?wpfpaction=trash&amp;page=1&amp;postid=". $post_id ."' rel='nofollow'>remove from list</a>";
        // $link = apply_filters( 'wpfp_remove_favorite', $link );
        echo $link;
    }
}

function wpfp_saveforlater_favorite_link($post_id) {
    if (wpfp_is_user_can_edit()) {
        $wpfp_options = wpfp_get_options();
        $class = 'wpfp-link remove-parent';
        $link = "<a id='rem_$post_id' class='$class fav-remove' href='?wpfpaction=save&amp;page=1&amp;postid=". $post_id ."' rel='nofollow'>save for later</a>";
        $link = apply_filters( 'wpfp_saveforlater_favorite_link', $link );
        echo $link;
    }
}
